...
if (! function_exists('pmpro_sequence_check_for_new_content')):
	...
	function pmpro_sequence_check_for_new_content( $sequenceId = 0)
	{
		...
		if ($sequenceId == 0) {
			$sequence->dbgOut('cron: ID is at default (0)');
			return;
		} else {
			$sequence->dbgOut('cron - Sequence ID:' . $sequenceId);
		}
		...
		foreach ( $seq as $s )
		{
			...
			if ( $seq_settings->sendNotice == 1 ) {
				$sequence->dbgOut( 'cron() - Sequence ' . $s->ID . ' is configured to send new content notices to users.' );
