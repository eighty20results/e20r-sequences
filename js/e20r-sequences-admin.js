...
(function($) {
    ...
    sequenceSettings = {
        init: function() {
            ...
            this.email_settings = $('.e20r-sequence-email');
            this.sendAlertCtl   = $('#e20r-sequence_sendnotice');
            this.checkboxes     = $('.e20r-sequence-settings-display .e20r-sequence-setting-col-1 > input[type="checkbox"]');
            this.saveBtn        = $('a[class^="save-e20rseq"]');
            this.cancelBtn      = $('a[class^="cancel-e20rseq"]');
            this.editBtn        = $('a.e20r-seq-edit');

            this.selects        = $('div.e20r-sequence-settings-input select');
            this.sortOrderCtl   = $('#e20r-sequence_sortorder');
            this.delayCtl       = $('#e20r-sequence_delaytype');
            this.showDelayCtl   = $('#e20r-sequence_showdelayas');
            this.templCtl       = $('#e20r-sequence_template');
            this.timeCtl        = $('#e20r-sequence_noticetime');
            this.dateCtl        = $('#e20r-sequence_dateformat');
            this.offsetCtl      = $('#e20r-sequence_offset');
            this.repeatCtl      = $('#e20r-sequence_allowRepeatPosts');
            this.sndAsCtl       = $('#e20r-sequence_sendas');

            this.addContentBtn  = $('#e20r_sequencesave');

            /* Input */
            this.excerptCtl     = $('#e20r-sequence_excerpt');
            this.subjCtl        = $('#e20r-sequence_subject');
            this.fromCtl        = $('#e20r-sequence_fromname');
            this.replyCtl       = $('#e20r-sequence_replyto');

            this.offsetChkCtl   = $('#e20r-sequence_offsetchk');

            this.spinner = $('div .seq_spinner');
        ...
        bind_buttons: function() {
            ...
            $class.editBtn.each( function() {
                ...
                $(this).unbind().on('click', function() {
                    $(this).closest('.e20r-sequence-settings-display').next('.e20r-sequence-settings-input').slideToggle();
                    $(this).slideToggle();
                });
            ...
            $("#e20r_seq_send").unbind().on('click', function() {
            ...
            $class.saveBtn.each(function() {
                ...
                $btn.unbind().on('click', function(){
                    ...
                    $class.hide_after_ok($(this));
                    window.console.log("Clicked the OK button");
                });
            });

            $class.addContentBtn.unbind('click').on('click', function() {
                postMeta.add_entry();
            });
            ...
            $class.cancelBtn.each(function() {
                ...
                $(this).unbind().on('click', function(){
                    ...
                    $(this).closest('.e20r-sequence-settings-input').slideToggle();
                    $(this).closest('.e20r-sequence-settings-input').prev('.e20r-sequence-settings-display').find('.e20r-seq-edit').slideToggle();
        ...
        change_delay_type: function() {
            ...
            var dtCtl = $('#e20r-sequence_delayType');

            var selected = dtCtl.val();
            var current = $('input[name=e20r_sequence_settings_hidden_delay]').val();

            if (dtCtl.val() !== $('#e20r_sequence_settings_hidden_delay').val()) {

                if (!confirm(e20r_sequence.lang.delay_change_confirmation)) {

                    dtCtl.val(current);
                    $('#hidden_e20r_seq_wipesequence').val(0);

                    return false;
                } else
                    $('#hidden_e20r_seq_wipesequence').val(1);
        ...
        delay_as_choice: function( $visibility ) {
            ...
            if ($visibility === 'hide') {
                ...
                $('.e20r-seq-showdelayas').hide(); // hide
                $('#e20r-seq-showdelayas-select').hide(); // hide
                $('.e20r-seq-delay-btns').hide(); // hide
            }
            else {
                $('.e20r-seq-showdelayas').show(); // show
                $('.e20r-seq-delay-btns').show(); // show
                $('#e20r-seq-showdelayas-select').show(); // hide
            }
        },
        set_error_message: function( $message ) {

            var errCtl = $('#e20r-seq-error');
        ...
        manageDelayLabels: function( $currentDelayType ) {
            ...
            var delayDlg = $('#e20r-seq-showdelayas');
            var delayEdit = $('#e20r-seq-edit-showdelayas');
        ...
        hide_rows: function() {
            ...
            $('.e20r-sequence-hidden').each(function() {
                $(this).hide();
            });
        },
        ...
        send_alert: function() {
            ...
            if ( $sequence === 'undefined' ) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    timeout: ( parseInt( e20r_sequence.ajaxtimeout ) * 1000 ),
                    dataType: 'JSON',
                    data: {
                        action: 'e20r_send_notices',
                        e20r_sequence_sendalert_nonce: $('#e20r_sequence_sendalert_nonce').val(),
        ...
        set_responsive: function() {
            ...
            for ( i = 0; i < headers.length; i++) {
                headertext[i]=[];
                for ( j = 0; head_row = headers[i].rows[0].cells[j]; j++) {

                    headertext[i].push(head_row.textContent);
                }
            }

            for (var h = 0, tbody; tbody = tablebody[h]; h++) {
                for ( i = 0; row = tbody.rows[i]; i++) {
    ...
    postMeta = {
        init: function() {
            ...
            this.sequence_list = $( 'select.e20r_seq-memberof-sequences');
            this.spinner = $('div.seq_spinner');
            this.delay_type     = $('#e20r-seq-hidden-delay-type').val();
        ...
        bind_controls: function() {
            ...
            $class.sequence_list = $( 'select.e20r_seq-memberof-sequences');

            $('select.new-sequence-select, .e20r_seq-memberof-sequences').each(function() {
            ...
            $("button.e20r-sequences-clear-cache").unbind('click').on('click', function() {

                $class.clear_cache();
            });

            $("#e20r-seq-new-meta").unbind('click').on( "click", function() {

                $class.manage_meta_rows();
                window.console.log("Add new table row for metabox");
                $class.spinner.show();

                // $class.row_visibility( $( '.new-sequence-select' ), 'select' );
            ...
            $('.delay-row-input input:checkbox').unbind('click').on( 'click', function() {
            ...
            $('.delay-row-input.sequence-delay button.e20r-sequence-remove-alert').unbind('click').on('click', function() {
        ...
        clear_post_notice_alerts: function( sequence_id, post_id, delay ) {
            ...
            var data = {
                ...
                'e20r_sequence_postmeta_nonce': $('#e20r_sequence_postmeta_nonce').val(),
        ...
        remove_entry: function( post_id, delay ) {
            ...
            if ( $('#e20r_sequence_meta').length ) {
                in_admin_panel = true;
            }
            $('#e20r_sequencesave').attr('disabled', 'disabled');
            // $('#e20r_sequencesave').html(e20r_sequence.lang.saving);
            ...
            $.ajax({
                ...
                data: {
                    ...
                    e20r_sequence_id: $('#e20r_sequence_id').val(),
                    in_admin_panel: in_admin_panel,
                    e20r_seq_post: post_id,
                    e20r_seq_delay: delay,
                    e20r_sequence_post_nonce: $('#e20r_sequence_post_nonce').val()
                },
                ...
                success: function($data){
                    ...
                    if ($data.data.html !== null && $data.data.html.length) {
                        $('#e20r_sequence_posts').html( $data.data.html );
                    }
                    ...
                    if (typeof $data.data === 'object') {
                        ...
                        if ( undefined != $data.data.message ) {
                ...
                complete: function() {
                    ...
                    $('#e20r_sequencesave').removeAttr('disabled');
                    $class.bind_controls();
                }
            });
        },
        edit_entry: function(post_id, delay) {
            ...
            $('#newmeta').focus();
            $('#e20r_sequencepost').val(post_id).trigger("change");
            $('#e20r_sequencedelay').val(delay);
            $('#e20r_sequencesave').html(e20r_sequence.lang.save);
        },
        ...
        add_entry: function() {
            ...
            var saveBtn = $('#e20r_sequencesave');
            var $class = this;

            if ('' === $('#e20r_sequence_post').val() || undefined !== saveBtn.attr('disabled')) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    e20r_sequence_id: $('#e20r_sequence_id').val(),
                    e20r_sequence_post: $('#e20r_sequencepost').val(),
                    e20r_sequence_delay: $('#e20r_sequencedelay').val(),
                    e20r_sequence_post_nonce: $('#e20r_sequence_post_nonce').val()
                },
                error: function( $response, $errString, $errType ) {
                    ...
                    if ($errString === 'timeout') {
                        $class.set_error_message("Timeout: Unable to add post/page (ID: " + $("#e20r_sequencepost").val() + ")");
                        return;
                    }
                    ...
                    if (typeof $response.data === 'object') {
                        ...
                        if ( undefined != $response.data.message ) {
                            var last_element = $response.data.length - 1;
                            window.console.log("Received " + $response.data.length + " error messages");
                            $class.set_error_message($response.data[last_element].message);
                        }
                ...
                success: function($returned, $success ){
                    ...
                    if ($returned.data.html !== null && $returned.data.html.length) {
                        ...
                        $('#e20r_sequence_posts').html($returned.data.html);
                    }
                    ...
                    if (($returned.data.html !== null)|| ($returned.data.message !== null)) {
                        ...
                        if (typeof $returned.data === 'object') {
                            ...
                            window.console.log("Received an object as the error status", $returned.data );

                            if ( undefined != $returned.data.message ) {
        ...
        remove_sequence: function( $me ) {
            ...
            $.ajax({
                ...
                success: function ($data) {
                    ...
                    if ($data.data) {
                        $('#e20r_seq-configure-sequence').html($data.data);
                    }
        ...
        show_controls: function() {
            ...
            if ( $('div#e20r_seq-configure-sequence').length === 0) {
                return;
            }
            ...
            if ( $count >= 1 ) {
                ...
                $class.row_visibility( $( 'select.new-sequence-select') , 'none' );

                $('#e20r-seq-new').show();
                $('#e20r-seq-new-meta').show();
                $('#e20r-seq-new-meta-reset').hide();
            }
            else {

                // Show the row for the 'Not defined' in the New sequence drop-down
                $class.row_visibility( $( 'select.new-sequence-select' ), 'select' );
        ...
        manage_meta_rows: function() {
            ...
            $( '.e20r_seq-memberof-sequences, .new-sequence-select, .e20r-seq-delay-info, .e20r_seq-remove-seq' ).each( function() {
            ...
            $( '#e20r-seq-new-meta' ).attr( 'disabled', true );
            $( '#e20r-seq-new-meta-reset' ).attr( 'disabled', true );
        },
        row_visibility: function( $element, $show ) {

            var $selectLabelRow = $($element).parent().parent().prev();
            var $selectRow = $($element).parent().parent();

            var $delayLabelRow = $($element).parent().parent().next();
            var $delayRow = $($delayLabelRow).next();
        ...
        add_sequence_post_row: function( ) {
            ...
            var table = $("#e20r-seq-metatable").find('tbody');
        ...
        /* _duplicate_meta_entry: function( sequence_id, delay_val, entry ) {
            ...
            $("tr.select-row-input.sequence-select select.e20r_seq-memberof-sequences").each( function() {
            ...
            if ( ( null !== select_label ) || ( false !== select_label ) ) {
                ...
                window.console.log("Find the last delay value input in the list: ", $("tr.delay-row-input.sequence-delay:last") );

                $("tr.delay-row-input.sequence-delay:last").appendTo(select_label);
                window.console.log("Attempted to add select label: ", select_label );
                $("tr.select-row-label.sequence-select-label:last").appendTo(select);
                $("tr.select-row-input.sequence-select:last").appendTo(delay_label);
                $("tr.delay-row-label.sequence-delay-label:last").appendTo(delay);
        ...
        meta_select_changed: function( $self ) {
            ...
            $class.manage_meta_rows();
            $class.bind_controls();
            $( '#e20r-seq-new-meta' ).attr( 'disabled', false );
            $( '#e20r-seq-new-meta-reset' ).attr( 'disabled', false );
        ...
        _set_labels: function() {
            ...
            var delayType = $('#e20r_sequence_delaytype').val();
            ...
            $('#e20r_sequence_delaylabel').html( headerHTML_start + labelText + headerHTML_end);
            $('#e20r_sequence_delayentrylabel').html( entryHTML_start + entryText + entryHTML_end);
        }
    };

    jQuery(document).ready(function($){

        var error_sel = $('div#e20r-seq-error');
        ...
        if ( $(".wp-admin table#e20r_sequencetable").length !== 0) {
